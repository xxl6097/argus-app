...
| `backup_handlers.go` | `/api/backup/{export,import}` | tar.gz 打包下载 / 上传恢复 (核心逻辑在 `backup.go`) |
| `kick.go` | `/api/devices/kick` | **核心 + handler 同文件**: `kickStation()` 命令链 + `handleDeviceKick`; 仅 WiFi |
| `system.go` | `/api/system/{reboot,restart-network}` | **核心 + handler 同文件**: shell out 到 `/sbin/reboot` / `/etc/init.d/network restart` |
...
| 文件 | 覆盖 |
|---|---|
| `history_test.go` | `ComputeWorktime` 全部边界 (workday/weekend/holiday/makeup/otday + late/early/missing_in/missing_out) + `OverrideStore` 扁平 → 嵌套迁移 + `HolidayStore` 优先级 |
| `notify_test.go` | per-device 派发链路: `Set` 永不删 (只 `Delete` 删) + `notifications.json` 强制 `0600` |
| `credentials_test.go` | bcrypt 哈希 + 0600 文件权限 + 改密重新落盘 + `SessionStore` issue/validate/revoke/expire + RevokeAll |
| `version_test.go` | semver 比较 (含 pre-release / 反向 / 等价等 8 case) + GitHub API 缓存 + gh-proxy.com fallback |
| `backup_test.go` | round-trip + `restore_credentials=false` 跳过 + 3 种 zip-slip 拒绝 + 缺/错 manifest |
| `punch_classify_test.go` | dispatchNotify 的打卡分类四象限 (NotPunch / CheckIn / CheckOut / Transient) + 跨日不污染 |
| `punch_checkout_test.go` | recordPunchCheckout 7 个分支 + last-write-wins + 已有 `in` 不覆盖 |
...
模块对外暴露的是 `web.NewServer(watcher, opts...) *web.Server` + 一组
`Option` 函数 (在 `server.go` 顶部, 比如 `WithAliases` / `WithSettings`
/ `WithCredentials` / `WithVersion` / ...)。`main.go` 把命令行参数翻译
成这些 Option, 然后:
...
srv := web.NewServer(watcher,
    ...
    web.WithSettings(settingsStore),
    web.WithCredentials(credStore),
    web.WithVersion(versionInfo, "xxl6097/argus-app"),
    // ...
)
http.ListenAndServe(*listen, srv)

// Watcher 的事件回调要把 srv.OnEvent 串进去:
watcher.Run(ctx, srv.OnEvent, onError)
// syslog 可选, 串进来能让 history 落 src=syslog:WPA_COMPLETE 之类:
go owrt.WatchSyslog(ctx, srv.OnSyslog, onError)
```

完整 18 个 Option / flag 见 `cmd/app/main.go` 顶部 + `argus-app -help` 输出。
